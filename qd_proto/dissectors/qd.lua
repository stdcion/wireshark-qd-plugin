...
qd.ws_fields = {
    ...
    msg_type = ProtoField.uint8("qd.msg_type", "Type", base.DEC,
                                utils.enum_tbl_to_str_tbl(data_struct.qd_type))
}
...
local function read_msg_type(buf, off)
    ...
    qd_message_type.val_str = utils.enum_val_to_str(data_struct.qd_type,
...
function qd.dissect(proto, tvb_buf, off, packet_info, tree)
    ...
    if (len == 0) then
        ...
        subtree:add(qd.ws_fields.msg_type, data_struct.qd_type.HEARTBEAT)
